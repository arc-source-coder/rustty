...
fn test_scroll_viewport() {
    ...
    let newlines = "\n".repeat(50);
    unsafe { ghostty_terminal_feed(ptr, newlines.as_ptr(), newlines.len()) };
    // These should not crash — scroll up, to top, to bottom
    unsafe { ghostty_terminal_scroll_viewport(ptr, -5) };
    unsafe { ghostty_terminal_scroll_viewport_top(ptr) };
    unsafe { ghostty_terminal_scroll_viewport_bottom(ptr) };
