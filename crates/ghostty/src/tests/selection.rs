...
fn test_selection_set_clear() {
    ...
    let rc = unsafe { ghostty_terminal_set_selection(ptr, 0, 0, 4, 0, 0) };
    assert_eq!(rc, 0);
    ...
    // Get selection text
    let mut len: usize = 0;
    let text_ptr = unsafe { ghostty_terminal_get_selection_text(ptr, &mut len) };
    assert!(!text_ptr.is_null());
    let selected =
        unsafe { std::str::from_utf8(std::slice::from_raw_parts(text_ptr, len)).unwrap() };
    assert_eq!(selected, "Hello");
    unsafe { ghostty_terminal_bytes_free(text_ptr, len) };

    // Clear selection
    unsafe { ghostty_terminal_clear_selection(ptr) };
    unsafe { ghostty_terminal_render_update(ptr) };
    let has = unsafe { ghostty_terminal_render_row_selection(ptr, 0, &mut sx, &mut ex) };
    assert_eq!(has, 0);
