...
fn test_bracketed_paste_enabled() {
    ...
    assert_eq!(unsafe { ghostty_terminal_is_bracketed_paste(ptr) }, 1);
    // CSI ?2004l disables it
    let seq_off = b"\x1b[?2004l";
    unsafe { ghostty_terminal_feed(ptr, seq_off.as_ptr(), seq_off.len()) };
    assert_eq!(unsafe { ghostty_terminal_is_bracketed_paste(ptr) }, 0);
    unsafe { ghostty_terminal_free(ptr) };
}
...
fn test_mouse_mode_enabled() {
    ...
    let seq = b"\x1b[?1003h";
    unsafe { ghostty_terminal_feed(ptr, seq.as_ptr(), seq.len()) };
    assert_eq!(unsafe { ghostty_terminal_get_mouse_mode(ptr) }, 4); // any=4
    // CSI ?1006h enables SGR format
    let seq2 = b"\x1b[?1006h";
    unsafe { ghostty_terminal_feed(ptr, seq2.as_ptr(), seq2.len()) };
    assert_eq!(unsafe { ghostty_terminal_get_mouse_format(ptr) }, 2); // sgr=2
    unsafe { ghostty_terminal_free(ptr) };
}
