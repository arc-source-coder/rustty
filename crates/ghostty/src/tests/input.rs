...
fn test_encode_key_basic() {
    ...
    let opts = unsafe { ghostty_terminal_get_input_opts(ptr) };
...
fn test_encode_mouse_sgr() {
    ...
    let n =
        unsafe { ghostty_terminal_encode_mouse(opts, 0, 0, 0, 5, 10, buf.as_mut_ptr(), buf.len()) };
    assert!(n > 0);
    ...
    let n =
        unsafe { ghostty_terminal_encode_mouse(opts, 0, 1, 0, 5, 10, buf.as_mut_ptr(), buf.len()) };
    let output = std::str::from_utf8(&buf[..n]).unwrap();
    assert_eq!(output, "\x1b[<0;6;11m");
...
fn test_encode_mouse_x10() {
    ...
    let n =
        unsafe { ghostty_terminal_encode_mouse(opts, 0, 0, 0, 0, 0, buf.as_mut_ptr(), buf.len()) };
    assert!(n > 0);
...
fn test_encode_paste_bracketed() {
    ...
    unsafe { ghostty_terminal_feed(ptr, b"\x1b[?2004h".as_ptr(), 8) };
    ...
    let opts = InputOpts {
        ...
        mouse_event: MouseMode::from_raw(opts.mouse_event),
        mouse_format: MouseFormat::from_raw(opts.mouse_format),
        bracketed_paste: opts.bracketed_paste != 0,
        focus_event_mode: opts.focus_event_mode != 0,
    };

    let mut buf = [0u8; 64];
    let n = encode_paste(opts, b"hello", &mut buf);
    assert_eq!(&buf[..n], b"\x1b[200~hello\x1b[201~");

    unsafe { ghostty_terminal_free(ptr) };
}

#[test]
fn test_encode_paste_empty() {
    let opts = InputOpts {
    ...
    let mut buf = [0u8; 16];
    let n = encode_paste(opts, b"", &mut buf);
    assert_eq!(n, 0);
}
